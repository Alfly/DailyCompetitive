...
pair<int, int> trimLeftRight(vector<int>& height) {
    int n = height.size();
    int l = 0, r = n - 1;
    while (l < r && height[l] <= height[l + 1]) height[l++] = 0;
    while (l < r && height[r] <= height[r - 1]) height[r--] = 0;
    return {l, r};
}
...
public:
    int trap(vector<int>& height) {
        ...
        for (int i = 1; i < n - 1; i++) {
            ...
            // 两端都大于当前列的高度才会有水
            ans += max(min(hLeft, hRight) - height[i], 0);
        }
        return ans;
    }
};
...
public:
    ...
    int trap(vector<int>& height) {
        ...
        for (int i = l + 1; i <= r - 1; i++) {
            int hLeft = 0;   // left max height
            for (int j = l; j < i; j++) hLeft = max(hLeft, height[j]);
            int hRight = 0;  // right max height
            for (int j = r; j > i; j--)  hRight = max(hRight, height[j]);
            // 两端都大于当前列的高度才会有水
            ans += max(min(hLeft, hRight) - height[i], 0);
        }
        return ans;
...
对于解法二，求每个元素左右的最值时，都会涉及很多**重复计算**。用动态规划优化至10ms，再使用两端剔除可到 3ms。
...
计算每个柱子能接多少雨水，只需要知道左右两侧最高柱子的**最小值**。
    ...
    int trap(vector<int>& height) {
        ...
        while (l <= r) {
            if (maxLeft <= maxRight) {  // 左指针向右
                ans += max(maxLeft - height[l], 0);
                maxLeft = max(maxLeft, height[l]);
                l++;
            } else {    // 右指针向左
                ans += max(maxRight - height[r], 0);
...
注意：第 3 步弹出栈顶元素后，必须**判断栈是否为空**。只有弹出一个元素后栈不为空，才能保证弹出元素左侧有比它更高的元素，这时才能接到雨水。
